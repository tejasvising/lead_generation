...
const EWSPopupHigh = () => {
 ...
 // const highData=data.filter(row=> row.ewsRating=='High');
  useEffect(() => {
    const fetchData = async () => {
        try {
            const response = await fetch(`${API_URL}/donutReport/${window.parent.userName}/high`); //${window.parent.userName}
            if (!response.ok) {
